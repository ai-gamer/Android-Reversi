...
//index是为了棋子翻转的效果，0为黑色正面，11为白色正面
...
//问题是arraylist add函数是添加引用，网上推荐每次循环new一个新的对象，但是这里是函数调用无法解决
    public List<Undo> undo=new ArrayList<Undo>();
    public List<byte[][]> chessBoardState=new ArrayList<byte[][]>();
    private byte[][] firstChessBoardState=new byte[M][M];
    private void InitialBoardState(){
        firstChessBoardState[3][3]=WHITE;
        firstChessBoardState[3][4]=BLACK;
        firstChessBoardState[4][3]=BLACK;
        firstChessBoardState[4][4]=WHITE;
        chessBoardState.add(firstChessBoardState);
    }

    public void move(byte[][]chessBoard, List<Move> reversed,Move move,int chessColor) {
        byte[][] chessBoardNow=new byte[8][8];
        for(int i=0;i<8;i++){
            for(int j=0;j<8;j++){
                chessBoardNow[i][j]=chessBoard[i][j];
            }
        }
        chessBoardState.add(chessBoardNow);
        Util.copyBinaryArray(chessBoard, this.chessBoard);
        //reversed中存在所有需要被翻转move的信息，依次进行翻转
        for (int i = 0; i < reversed.size(); i++) {
            ...
            //之前忘记写了 用于改变ai落子位置的棋子的颜色
            int row = move.row, col = move.col;
            //将位子位置的棋子对应index改为对应图片
    ...
    public int getRow(float y){return(int)Math.floor((y-chessBoardTop)/a);}

    public int getCol(float x){return(int)Math.floor((x-chessBoardLeft)/a);}
